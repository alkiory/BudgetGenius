...
const customToast = ({ message, type = 'info', duration = 5000 }: CustomToastProps) => {
  const toastStyles = {
    warning: {
...
export const confirmToast = (message: string, options: ConfirmToastOptions, duration?: number) => {
  return toast.custom(
    (t) => (
      <div
        className={`${t.visible ? 'animate-enter' : 'animate-leave'}

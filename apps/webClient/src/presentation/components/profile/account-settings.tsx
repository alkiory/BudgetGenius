...
export function AccountSettings() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (settingsToUpdate.timezone === settings?.timezone && settingsToUpdate.currency === settings?.currency && settingsToUpdate.locale === settings?.locale) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <CardContent className="space-y-4">
            ...
            <Select
              ...
              onChange={(value) => {
                setSettingsToUpdate((prev) => ({ ...prev, currency: value as Currency }))
              }}
              options={[
          ...
          <CardFooter>
            <Button type="submit" disabled={isSuccess}>{t('settings.savePreferences')}</Button>
          </CardFooter>
        </form>
      </Card>
            ...
            <div className="space-y-4">
              <div className="rounded-md bg-red-50 p-4 dark:bg-red-900/20">
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-red-600 dark:text-red-500">{t('settings.warning')}</h3>
                    <div className="mt-2 text-sm text-red-600/80 dark:text-red-500/80">
                      <p>
                        {t('profile.deleteConfirmationWarning')}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="confirm-delete" className="text-red-600 dark:text-red-500">                    {t('settings.typeToConfirm')}
                </Label>
                <Input
                  id="confirm-delete"
                  value={confirmText}

...
export function BudgetList({ onBudgetSelect, onEditBudget, onDeleteBudget, budgets, isLoading }: BudgetListProps) {
      ...
      {budgets && budgets.length > 0 ? (
        <div className="space-y-2">
          {budgets.map((budget) => {
            ...
            const percentSpent = budget.totalAllocated > 0 ? (budget.totalSpent / budget.totalAllocated) * 100 : 0
              ...
              >
                ...
                <div className="mt-4">
                  <div className="flex items-center justify-between text-sm">
                    <span>
                      ${formattedSpent.formatted.split('+')[1]} of ${formattedAllocated.formatted.split('+')[1]}
                    </span>
                ...
                <div className="mt-2 text-sm">
                  ...
                  >
                    {remaining >= 0
                      ? `${formattedRemaining.formatted.split('+')[1]} ${t('budgets.remaining')}`
                      : `${Math.abs(formattedRemaining.amount).toFixed(2)} ${t('dashboard.overBudget')}`}
                  </span>
                </div>
              </div>
            )

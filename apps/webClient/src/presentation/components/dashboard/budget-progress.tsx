  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-sm dark:bg-slate-800">
      ...
      <div className="space-y-4">
        {isLoading && <Loader />}
        {isSuccess && budgets?.length > 0 ? budgets
          ?.slice(-1 * 3)
          ?.map((budget) => {
            const percentage = Math.min(Math.round((budget.totalSpent / budget.totalAllocated) * 100), 100)
            ...
            return (
              <div key={budget.id}>
                ...
                <div className="h-2 w-full rounded-full bg-slate-100 dark:bg-slate-700">
                  <div
                    className={`h-2 rounded-full progress-shim ${isOverBudget ? "bg-red-500" : getColorBasedOnEndDate(budget.endDate)}`}
                    style={{ width: `${percentage}%` }}
                  ></div>
                </div>
                <div className="mt-1 flex justify-between text-xs">
                  <span
                    className={isOverBudget ? "text-red-500 dark:text-red-400" : "text-slate-500 dark:text-slate-400"}
                  >
                    {percentage}% {isOverBudget && t('dashboard.overBudget')}
                  </span>
                  <span className="text-slate-500 dark:text-slate-400">
                    {Math.max(formattedAllocated.amount - formattedSpent.amount, 0)} {t('dashboard.left')}
                  </span>
                </div>
              </div>
            )
          ...
          <div className="flex items-center justify-center">
            <span className="text-muted-foreground">{t('dashboard.noBudgetsYet')}</span>
          </div>
        )}
      </div>
    </div>

...
export function ExpenseCategories({ total, byCategory, largest, timezone, period }: Props) {
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-sm dark:bg-slate-800">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold">{t('dashboard.expenseBreakdown')}</h2>
            ...
            <PieChart>
              ...
              >
                {byCategory?.map((entry, i) => (
                  <Cell key={i} fill={COLORS[i % COLORS.length]} alphabetic={entry} />
                ))}
              </Pie>
              <Tooltip formatter={(v) => [`${v.toLocaleString(formattedLocale, {
                timeZone: timezone
              })}`, 'Amount']} />
              <Legend />
            </PieChart>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-md bg-slate-50 p-3 dark:bg-slate-700">
          <p className="text-sm text-slate-500">{t('dashboard.totalExpenses')}</p>
          <p className="text-xl font-bold">{total}</p>
        </div>
        <div className="rounded-md bg-slate-50 p-3 dark:bg-slate-700">
          <p className="text-sm text-slate-500">{t('dashboard.largestCategory')}</p>

...
import {
  DollarSign, ShoppingBag, Home, Utensils,
  Heart, Car, ShoppingCart, Banknote, Pizza, Film,
  Plus, Wallet, Landmark, Bus, Wifi, Activity
} from "lucide-react";
...
const CATEGORY_STYLES: Record<string, { color: string; bg: string }> = {
  ...
  "Transportation": { color: "text-cyan-500", bg: "bg-cyan-100 dark:bg-cyan-900" },
  "Entertainment": { color: "text-pink-500", bg: "bg-pink-100 dark:bg-pink-900" },
  "Utilities": { color: "text-yellow-500", bg: "bg-yellow-100 dark:bg-yellow-900" },
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-sm dark:bg-slate-800">
      ...
      <div className="space-y-4">
        {isSuccess && transactions.transactions?.length > 0 ? transactions.transactions
          ?.slice(-1 * 3)
          ?.map((transaction) => {
            const { IconComponent, color, bg } = getTransactionIcon(transaction.category);
          ...
          <div className="flex items-center justify-between rounded-lg border border-slate-100 p-3 dark:border-slate-700">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center justify-center">
                <span className="text-muted-foreground">{t('dashboard.noTransactionsYet')}</span>
              </div>
            </div>
          </div>
        )}

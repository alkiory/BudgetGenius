...
export function GoalModal({ isOpen, onClose, goal, refetchParent }: GoalModalProps) {
  ...
  const { mutate: addGoal } = useMutation({
    ...
    onSuccess: (data: any) => {
      successToast(data?._offline ? data.message : "Goal created successfully", 3000, "goal-create")
  ...
  const { mutate: updateGoal } = useMutation({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    onSuccess: (data: any) => {
      successToast(data?._offline ? data.message : "Goal updated successfully", 3000, "goal-update")
      queryClient.invalidateQueries({ queryKey: ["dashboard"] })
      if (!data?._offline) {
        refetchParent();
      }

...
import { Transaction } from "@domain/dashboard/transactions/transaction.entity";
  ...
  export const createTransaction = async ({dto}: {dto: Omit<Transaction, "id">}) => {
    return await HttpTransactionRepository.createTransaction({dto});
  }

  export const updateTransaction = async ({ dto }: { dto: Partial<Transaction> }) => {
    return await HttpTransactionRepository.updateTransaction({ dto });
  }

  export const deleteAllTransactions = async ({ transactionId }: {transactionId: number[]}) => {
    return await HttpTransactionRepository.deleteAllTransactions(transactionId);
  }

  export const deleteTransaction = async ({ transactionId }: { transactionId: number }) => {

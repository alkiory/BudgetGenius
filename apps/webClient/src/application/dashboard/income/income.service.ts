...
export const createIncome = async ({dto}: {dto: Omit<Transaction, "id">}) => {
...
export const deleteIncome = async ({ transactionId }: { transactionId: number }) => {
  return await HttpTransactionRepository.deleteTransaction(transactionId);
}

export const deleteAllIncomes = async ({transactionId }: {transactionId: number[]}) => {

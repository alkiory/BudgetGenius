...
test.describe("Offline Queue", () => {
  ...
  test.afterEach(async ({ page }) => {
    await page
      .evaluate(() => localStorage.removeItem("offline_queue"))
      .catch(() => {});
  });

  test("queues a transaction when offline and flushes it on reconnect", async ({
    page,
  }) => {
    // ---- Mock auth endpoints ----
  ...
  test("shows offline indicator when navigator.onLine is false", async ({
    page,
  }) => {

...
test.describe("i18n: Legal and Contact Pages", () => {
  ...
  test.describe("Privacy Policy (/privacy-policy)", () => {
    test("switches between English and Spanish via the language switcher", async ({
      page,
    }) => {
      ...
      await expect(
        page.getByText("Information We Collect").first(),
      ).toBeVisible();
      await expect(page.getByText("How We Use Your Information")).toBeVisible();
      await expect(
        page.getByText("At Budget Genius, we take your privacy seriously"),
      ).toBeVisible();
      ...
      await expect(
        page.getByText("Información que Recopilamos").first(),
      ).toBeVisible();
  ...
  test.describe("Terms of Service (/terms-and-conditions)", () => {
    test("switches between English and Spanish via the language switcher", async ({
      ...
      await expect(
        page.getByText("These Terms of Service").first(),
      ).toBeVisible();
      await expect(page.getByText("Back to Home")).toBeVisible();
      ...
      await expect(
        page.getByRole("heading", { name: "Términos del Servicio" }),
      ).toBeVisible();
  ...
  test.describe("Contact Sales (/contact-sales)", () => {
    test("switches between English and Spanish via the language switcher", async ({
      ...
      // --- English ---
      await expect(
        page.getByRole("heading", { name: "Contact Sales" }),
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("heading", { name: "Contactar Ventas" }),
      ).toBeVisible();
      await expect(
        page.getByText("Seguridad de nivel empresarial"),
      ).toBeVisible();
